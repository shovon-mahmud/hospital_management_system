...
export default function DoctorsPage() {
  ...
  const [doctorAvailability, setDoctorAvailability] = useState({}) // Store availability by doctorId
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Try to load departments (Admin/HR only)
      try {
        const resDept = await api.get('/departments?limit=200')
  ...
  const fetchDoctorAvailability = async (doctorId) => {
    try {
      ...
      setDoctorAvailability(prev => ({ ...prev, [doctorId]: availabilityData }))
    } catch (e) {
      console.error('Failed to load availability', e)
      setDoctorAvailability(prev => ({ ...prev, [doctorId]: [] }))
    }
  }

  const toggleExpanded = async (doctorId) => {
    if (expandedId === doctorId) {
      ...
      if (!doctorAvailability[doctorId]) {
        await fetchDoctorAvailability(doctorId)
      }
    }
  }
                ...
                <li key={d._id} className="p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors border border-transparent hover:border-gray-200 dark:hover:border-gray-600">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      {(d.availability?.length>0) && (
                        <div className="mt-2 flex flex-wrap gap-1">
                          {d.availability.slice(0,7).map((a, idx) => (
                            <span key={idx} className="text-[10px] px-2 py-0.5 rounded-full bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-200">{a.day}: {(a.slots||[]).map(s=>`${s.start}-${s.end}`).join(', ')}</span>
                          ))}
                        </div>
                      )}
                    </div>
                    <div className="flex flex-col sm:flex-row gap-2 shrink-0">
                      ...
                      <button onClick={()=>toggleExpanded(d._id)} className="px-3 py-1.5 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200">{expandedId===d._id?'Hide':'Details'}</button>
                  ...
                  {expandedId===d._id && (
                    <div className="mt-3 grid md:grid-cols-2 gap-3 text-xs text-gray-700 dark:text-gray-300">
                      ...
                      <div className="p-3 rounded-lg bg-gray-50 dark:bg-gray-800/60 border border-gray-200 dark:border-gray-700">
                        ...
                        {(() => {
                          const availability = doctorAvailability[d._id]
                          if (!availability) {
                            return <p className="text-xs text-gray-500">Loading...</p>
                          }
                          if (availability.length === 0) {
                            return <p className="text-xs text-gray-500">No schedule configured</p>
                          }
                          return (
                            <ul className="space-y-1 text-xs">
                              {availability.map((a, i) => (
                                <li key={i} className="flex items-center gap-2">
                                  <span className="font-medium text-blue-600 dark:text-blue-400 min-w-[4rem]">{a.dayOfWeek}</span>
                                  <span className="text-gray-700 dark:text-gray-300">
                                    {a.workingHours?.start} - {a.workingHours?.end}
                                  </span>
                                  {a.breaks?.length > 0 && (
                                    <span className="text-[10px] text-gray-500">(Break: {a.breaks[0].start}-{a.breaks[0].end})</span>
                                  )}
                                </li>
                              ))}
                            </ul>
