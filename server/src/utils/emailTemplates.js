...
export const verifyEmailCodeTemplate = ({ name, verificationCode, expiresInMinutes = 15 }) => {
  ...
  .code { font-size:32px; font-weight:700; letter-spacing:8px; color:#2563eb; background:#eff6ff; padding:16px 24px; border-radius:8px; text-align:center; margin:24px 0; font-family:monospace; }
  .footer { font-size:12px; color:#64748b; margin-top:32px; }
  </style></head><body><div class="container">
  <h1>Verify your account</h1>
  <p>Hi ${safeName || 'there'},</p>
  <p>Thanks for signing up. Please use the verification code below to activate your HMS account.</p>
  <div class="code">${escapeHtml(verificationCode)}</div>
  <p>Enter this code in the verification page to complete your registration.</p>
  <p>This code will expire in ${expiresInMinutes} minutes for security.</p>
  <div class="footer">
    <p>If you didn't create an account, you can safely ignore this email.</p>
  </div>
  </div></body></html>`;
  const text = `Verify your HMS account\n\nHi ${safeName || 'there'},\n\nYour verification code is: ${verificationCode}\n\nEnter this code to verify your account (expires in ${expiresInMinutes} minutes).\n\nIf you did not create an account, ignore this email.`;
  return { html, text };
};
